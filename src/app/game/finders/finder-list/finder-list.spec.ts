...
describe('FinderList', () => {
  ...
  it('should render a table of finders with their data formatted correctly', async () => {
    ...
    expect(screen.getAllByRole('cell')).toHaveLength(finderList.length * 3);
    ...
    expect(screen.getByRole('row', { name: '1 Foo 1 second' })).toBeVisible();
    expect(screen.getByRole('row', { name: '2 Foo 9 seconds' })).toBeVisible();
    expect(screen.getByRole('row', { name: '3 Bar 1 minute' })).toBeVisible();
    expect(screen.getByRole('row', { name: '4 Bar 7 minutes' })).toBeVisible();
    expect(screen.getByRole('row', { name: '5 Baz 1 hour' })).toBeVisible();
    expect(screen.getByRole('row', { name: '6 Baz 5 hours' })).toBeVisible();
    expect(screen.getByRole('row', { name: '7 Tar 1 day' })).toBeVisible();
    expect(screen.getByRole('row', { name: '8 Tar 3 days' })).toBeVisible();
  });
});
